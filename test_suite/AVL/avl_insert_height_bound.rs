...
mod avl_insert_height_bound {
    ...
    // ---- target theorem (F* §8, 명제 2) -----------------------------------
    // 삽입 후 트리의 높이는 이전과 같거나 정확히 1만 증가한다.
    #[val((x: Nat, t: Tree) -> Lemma(requires(is_balanced(t)), ensures(
        height(insert(x, t)) == height(t)
            || height(insert(x, t)) == Nat::S(Box::new(height(t))))))]
    fn insert_height_bound(x: Nat, t: Tree) {
        match t {
            Tree::Leaf => (),
            Tree::Node(l, v, _, r) => {
                if lt(x.clone(), v.clone()) {
                    insert_height_bound(x, *l);
                } else if lt(v.clone(), x.clone()) {
                    insert_height_bound(x, *r);
                } else {
                    ()
                }
            }
