// Trial file: the AVL specification (identical to avl_spec.rs) plus ONE target
// theorem with its induction skeleton (case split and recursive calls) and
// nothing else -- no helper lemmas, no hints. Copy into tests/ to work on it;
// this file itself is never edited during a trial.
#[ravencheck::module]
mod avl_insert_preserves_balance {
    ...
    // ---- target theorem (F* §8, 명제 1) -----------------------------------
    // 삽입 후 트리의 높이 균형이 유지된다.
    #[val((x: Nat, t: Tree) -> Lemma(requires(is_balanced(t)), ensures(is_balanced(insert(x, t)))))]
    fn insert_preserves_balance(x: Nat, t: Tree) {
        match t {
            Tree::Leaf => (),
            Tree::Node(l, v, _, r) => {
                if lt(x.clone(), v.clone()) {
                    insert_preserves_balance(x, *l);
                } else if lt(v.clone(), x.clone()) {
                    insert_preserves_balance(x, *r);
                } else {
                    ()
                }
            }
