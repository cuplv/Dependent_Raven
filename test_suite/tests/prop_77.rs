    ...
    fn tip_77(x: Nat, xs: NList) {
        match xs {
            NList::Nil => {
                ...
                instantiate!(NList::Cons(x, NList::Nil));
            ...
            NList::Cons(h, t) => {
                ...
                match *t.clone() {
                    ...
                    NList::Cons(h2, t2) => {
                        instantiate!(le(h, h2));
                        instantiate!(le(x, h2));
                        instantiate!(sorted(NList::Cons(h2, t2)));
                        instantiate!(NList::Cons(x, NList::Cons(h2, t2)));
                        instantiate!(sorted(NList::Cons(x, NList::Cons(h2, t2))));
                        instantiate!(insort(x, t2));
                        instantiate!(NList::Cons(h2, insort(x, t2)));
